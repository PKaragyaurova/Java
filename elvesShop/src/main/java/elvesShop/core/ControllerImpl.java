package elvesShop.core;

import elvesShop.common.ConstantMessages;
import elvesShop.common.ExceptionMessages;
import elvesShop.models.*;
import elvesShop.repositories.Repository;
...
public class ControllerImpl implements Controller {
    ...
    public String craftPresent(String presentName) {
        ...
        List<Helper> helpersCanWork = helperRepository.getModels().stream().filter(helper -> helper.getEnergy() > 50).collect(Collectors.toList());
        ...
        if (present.isDone()) {
            status = "done";
            return String.format(ConstantMessages.PRESENT_DONE, presentName, status) +
